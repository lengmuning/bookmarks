...
// Bundle ID for the bundled Safari extension. Replace before publishing.
let extensionBundleIdentifier = Bundle.main.bundleIdentifier.map { "\($0).Extension" }
    ?? "com.yourCompany.Safari-Bookmarks-Sync.Extension"
let sharedAppGroupIdentifier = "group.com.yourCompany.Safari-Bookmarks-Sync"
let sharedSyncConfigKey = "sync_config"
...
class ViewController: PlatformViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private var sharedDefaults: UserDefaults {
        UserDefaults(suiteName: sharedAppGroupIdentifier) ?? defaults
    }
#endif
    ...
    override func viewDidAppear() {
        ...
        view.window?.minSize = NSSize(width: 425, height: 325)
    }
#endif
    ...
    private func generatePair() async {
        do {
            ...
            if let now = pair.server_now {
                defaults.set(now, forKey: "last_sync")
            }
            publishExtensionConfig()
        ...
        do {
            ...
            for bookmark in bookmarks {
                ...
                if let envelope = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let now = envelope["server_now"] as? Double {
                    lastServerNow = now
                }
            }

            if lastServerNow > 0 {
                defaults.set(lastServerNow, forKey: "last_sync")
            } else {
    ...
    private func ensureSafariNotRunning() throws {
        let running = NSWorkspace.shared.runningApplications.contains { app in
            app.bundleIdentifier == "com.apple.Safari"
        }
        if running {
            throw AppError("Safari is running. Quit Safari first — writing to Bookmarks.plist while Safari is open may corrupt your bookmarks.")
        }
    }
    ...
    private func loadSafariBookmarks() throws -> [NativeBookmark] {
        ...
        var seen = Set<String>()
        collectBookmarks(from: root, folderPath: [], indexInParent: 0, into: &bookmarks, seen: &seen)
        return bookmarks
    }

    private func collectBookmarks(from node: [String: Any], folderPath: [String], indexInParent: Int, into bookmarks: inout [NativeBookmark], seen: inout Set<String>) {
        let type = node["WebBookmarkType"] as? String

        if type == "WebBookmarkTypeLeaf", let rawUrl = node["URLString"] as? String, !rawUrl.isEmpty {
            guard let canonical = canonicalUrl(rawUrl) else { return }
        ...
        for (index, child) in children.enumerated() {
            collectBookmarks(from: child, folderPath: nextPath, indexInParent: index, into: &bookmarks, seen: &seen)
        }
    }
    ...
    private func mergeRemoteBookmarksIntoSafari(_ remoteBookmarks: [RemoteBookmark]) throws -> Int {
        ...
        collectBookmarkLocationKeys(from: root, into: &existingKeys)
        let missing = missingRemoteBookmarks(remoteBookmarks, existingKeys: existingKeys)
        var added = 0
        for bookmark in missing {
            guard let canonical = canonicalUrl(bookmark.url) else { continue }
            ...
            root = addRemoteBookmark(
                title: (title?.isEmpty == false) ? title! : canonical,
                url: canonical,
                folderPath: folderPath,
                to: root,
                isRoot: true
            )
            existingKeys.insert(BookmarkLocationKey(canonicalUrl: canonical))
            added += 1
        }
        ...
        coordinator.coordinate(writingItemAt: access.url, options: [.forReplacing], error: &coordError) { url in
            do {
        ...
        if let coordError {
            throw AppError("Could not coordinate write to Safari bookmarks. Backup saved to \(backupURL.path). \(coordError.localizedDescription)")
        }
        if let writeError {
            throw AppError("Could not write Safari bookmarks. A backup was saved to \(backupURL.path). Grant Full Disk Access or choose a writable copy of Bookmarks.plist, then retry. \(writeError.localizedDescription)")
        }
        return added
    }
    ...
    private func missingRemoteBookmarks(_ remoteBookmarks: [RemoteBookmark], existingKeys: Set<BookmarkLocationKey>) -> [RemoteBookmark] {
        ...
        var result: [RemoteBookmark] = []
        for bookmark in remoteBookmarks {
            guard let canonical = canonicalUrl(bookmark.url) else { continue }
            let key = BookmarkLocationKey(canonicalUrl: canonical)
    ...
    private func collectBookmarkLocationKeys(from node: [String: Any], into keys: inout Set<BookmarkLocationKey>) {
        if let url = node["URLString"] as? String,
           let canonical = canonicalUrl(url) {
            keys.insert(BookmarkLocationKey(canonicalUrl: canonical))
        }
        guard let children = node["Children"] as? [[String: Any]] else { return }
        for child in children {
            collectBookmarkLocationKeys(from: child, into: &keys)
        }
    }
    ...
    private func backupBookmarksFile(_ url: URL) throws -> URL {
        ...
        let backupName = "Bookmarks.safari-sync-\(formatter.string(from: Date())).plist"
    ...
    private func publishExtensionConfig() {
        var config: [String: Any] = [:]
        config["api_url"] = defaults.string(forKey: "api_url") ?? ""
        config["pair_id"] = defaults.string(forKey: "pair_id") ?? ""
        config["device_id"] = defaults.string(forKey: "device_id") ?? ""
        config["device_token"] = defaults.string(forKey: "device_token") ?? ""
        config["code"] = defaults.string(forKey: "code") ?? ""
        config["last_sync"] = defaults.object(forKey: "last_sync") as? Double ?? 0
        config["last_remote_check"] = defaults.object(forKey: "last_remote_check") as? Double ?? 0
        config["pending_remote_count"] = defaults.object(forKey: "pending_remote_count") as? Int ?? 0
        config["auto_check_enabled"] = defaults.bool(forKey: "auto_check_enabled")

        sharedDefaults.set(config, forKey: sharedSyncConfigKey)
